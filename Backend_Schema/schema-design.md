...
Drafts expire 30 calendar days after being saved. This is enforced by a
manual check in `GET /api/forms/:id/draft/:resumeToken`, rather than a
MongoDB TTL index, so an expired draft returns `410 Gone` with
`{ "error": "This draft has expired" }`. Expired records are retained
until a separate cleanup policy is added.
