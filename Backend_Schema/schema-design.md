...
=======
The current implementation includes the Save Draft UI/state
serialization scaffold, but a backend save-draft route and
`resumeToken` generation implementation are not yet available
...
=======
| `partialValues` | object | Stores completed field values |
| `savedAt` | string | Records when the draft was saved |
| `resumeToken` | string | Intended identifier for resuming a draft |
>>>>>>> origin/main
...
=======
`partialValues` and the stored form schema when a draft is resumed.
>>>>>>> origin/main
...
=======
The exact `resumeToken` generation approach remains to be finalized
when the backend save-draft route is implemented.

The token should be generated by the backend and uniquely identify
the saved draft without exposing draft contents.
...
=======
## Day 25 — Sample Draft Seed Data

Sample saved drafts are included in the seed data for manual resume testing.

Three sample drafts are created with different completion states:

1. `day25-basic-incomplete-rahul`
   - Basic Information form
   - Only `fullName` is completed.

2. `day25-insurance-partial-amit`
   - Insurance Claim form
   - Partially completed.
   - `hasInsurance` is set to `Yes`.

3. `day25-3level-complete-demo`
   - Insurance Claim - Detailed form
   - Demonstrates a three-level conditional dependency chain:
     `hasInsurance → insuranceType → insuranceCompany → policyNumber`.
   - Includes completed branching values.

Draft data is stored in `partialValues` and can be loaded using
the draft `resumeToken`.

Manual resume verification can be performed through:
